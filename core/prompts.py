...
对每条候选句，依次判断以下 4 点，全部为"是"才判定 accept=true：

1. **学术评价性**：该句是否在评价/引述他人的学术贡献（如首次发现、开创性工作、里程碑、首次合成、最早提出等）？
   - ✗ 仅描述自己的实验操作步骤（"We first added the reagent..."）→ reject
   - ✗ 仅陈述客观事实而无评价/引述他人贡献的意味 → reject
   - ✓ 引述他人在某领域的开创性/首次/早期/突破性工作 → accept

2. **标志词语义正确**：标志词在此句中是否确实在描述**学术发展/贡献事件**？
   - ✗ 标志词描述的是实验操作顺序（"first" 指第一步操作而非首次发现）→ reject
   - ✗ 标志词是一般性时间叙述而非学术里程碑 → reject
   - ✓ 标志词描述某研究的首次/开创/里程碑意义 → accept

3. **三要素一致性**：标志词、被评作者、年份是否指向**同一个研究事件**？
   - ✗ 标志词描述的事件与作者的工作不是同一件 → reject
   - ✗ 年份与作者的学术贡献不相关 → reject
   - ✓ 三要素清晰指向同一研究事件 → accept

4. **被评对象确认**：被评对象是否为论文作者（而非机构、组织、项目等）？
   - ✗ 被评对象是机构/组织/项目而非具体论文作者 → reject
