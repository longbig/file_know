...
- pydantic v2（数据校验）
- rapidocr-onnxruntime（扫描版 PDF OCR，支持中文双栏布局）
...
│   ├── mineru_parser.py      # MinerU PDF 解析（高质量全文 + PyMuPDF 元数据）
...
扫描版 PDF OCR 支持（2026-06-02 新增）：
- 自动检测文字层为空的页面，降级走 RapidOCR（`rapidocr-onnxruntime`）
- 双栏布局处理：按文本块 x 坐标中心分左右栏，各自按 y 排序后拼接，避免左右栏交错
- OCR 引擎懒加载（`_get_ocr()`），首次调用时初始化，不影响非扫描版 PDF 性能
- 语言检测兼容：扫描版首页文字层为空时，用 OCR 后的 full_text 前段判断中英文
...
用 `magic-pdf` CLI 解析 PDF，生成 Markdown 全文，元数据由 PyMuPDF 提取（更准确）。接口与 `pdf_parser.py` 完全相同（`ParseResult`），可直接替换。

- `magic-pdf` 查找顺序：PATH → sys.executable 同目录 → 项目本地 venv（`venv312/venv/.venv`）
- 全文为 Markdown，支持复杂版式（双栏、公式、表格）
- 元数据：优先调 `pdf_parser.parse_pdf()` 提取，失败时降级用正则从 Markdown 提取
  - 修复前：MinerU Markdown 第一个 `#` 标题是期刊名（如 `Cell Reports`），被误为论文标题
  - 修复后（2026-06-16）：PyMuPDF 负责元数据，避免误提取
...
6. **扫描版 PDF OCR**（2026-06-02）：文字层为空时自动降级走 RapidOCR，支持双栏布局分栏拼接。
7. **中文作者锚定提取**（2026-06-02）：不依赖姓氏表，通过锚定信号（`等[N]`/`等（年份）`/`[N]`/括号引用）反向定位作者名，消除"方面的""程中风"等误匹配。
8. **MinerU 全文 + PyMuPDF 元数据混合**（2026-06-16）：`mineru_parser.py` 用 MinerU 提取高质量全文 Markdown，但元数据统一调用 `pdf_parser.parse_pdf()` 提取，避免 Markdown 首个 `#` 标题为期刊名导致标题/作者/期刊等字段全部出错。
